...
namespace BAL.Repositories
{
    public class SMPProductionDelayRepository
    {
            ...
            string createdBy)
        {
            ...
            {
                ProcessedRows =
                    ReadInt(result, "ProcessedRows"),

                InsertedPlantDelays =
                    ReadInt(result, "InsertedPlantDelays"),

                UpdatedPlantDelays =
                    ReadInt(result, "UpdatedPlantDelays"),

                DeactivatedPlantDelays =
                    ReadInt(result, "DeactivatedPlantDelays"),

                InsertedSMPProductionDelays =
                    ReadInt(result, "InsertedSMPProductionDelays"),

                UpdatedSMPProductionDelays =
                    ReadInt(result, "UpdatedSMPProductionDelays"),

                DeactivatedSMPProductionDelays =
                    ReadInt(result, "DeactivatedSMPProductionDelays"),

                InsertedFailureAnalyses =
                    ReadInt(result, "InsertedFailureAnalyses"),

                UpdatedFailureAnalyses =
                    ReadInt(result, "UpdatedFailureAnalyses"),

                GeneratedDelayCodes =
                    ReadInt(result, "GeneratedDelayCodes"),

                FirstGeneratedDelayCode =
                    ReadString(result, "FirstGeneratedDelayCode"),

                LastGeneratedDelayCode =
                    ReadString(result, "LastGeneratedDelayCode")
            };
        }
            ...
            IEnumerable<SMPProductionDelayUploadBLL> rows)
        {
            ...
            /*
             * Excel has 27 columns, but the TVP contains only fields
             * that the server is allowed to import.
             *
             * ID, DelayCode, StatusID and audit values stay DB-controlled.
             *
             * This exact order MUST match dbo.SMPDelayDaywiseUploadType.
             */
            ...
            table.Columns.Add("AgencyCode", typeof(string));
            ...
            foreach (SMPProductionDelayUploadBLL row in rows)
            {
                ...
                // Selected Plant ID / SMPPlantArea.AreaCode.
                dataRow["Plant"] =
                    DbValue(row.Plant);

                dataRow["ShiftGroup"] =
                    DbValue(row.ShiftGroup);

                dataRow["ProductionDate"] =
                    row.ProductionDate.Date;
                ...
                dataRow["AgencyCode"] =
                    DbValue(row.AgencyCode);

                dataRow["Area"] =
                    DbValue(row.Area);

                dataRow["Equipment"] =
                    DbValue(row.Equipment);

                dataRow["DelayDescription"] =
                    DbValue(row.DelayDescription);

                dataRow["ReasonForOccurrence"] =
                    DbValue(row.ReasonForOccurrence);

                dataRow["ActionTaken"] =
                    DbValue(row.ActionTaken);

                dataRow["LastPMDate"] =
                    row.LastPMDate.HasValue
                        ? (object)row.LastPMDate.Value.Date
                        : DBNull.Value;

                dataRow["FailureReportStatus"] =
                    DbValue(row.FailureReportStatus);

                dataRow["IncreaseMTBF"] =
                    DbValue(row.IncreaseMTBF);

                dataRow["DecreaseMTTR"] =
                    DbValue(row.DecreaseMTTR);

                dataRow["SAPBreakdownOrder"] =
                    DbValue(row.SAPBreakdownOrder);

                dataRow["FailureCategory1Component"] =
                    DbValue(row.FailureCategory1Component);

                dataRow["FailureCategory2RootCause"] =
                    DbValue(row.FailureCategory2RootCause);
